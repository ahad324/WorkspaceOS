...
fn bench_caches(c: &mut Criterion) {
    ...
    group.bench_function(
        BenchmarkId::new("LRU - Zipfian Reads", cache_capacity),
        |b| {
            b.iter_custom(|iters| {
    ...
    group.bench_function(
        BenchmarkId::new("Moka W-TinyLFU - Zipfian Reads", cache_capacity),
        |b| {
            b.iter_custom(|iters| {
                let mut elapsed = std::time::Duration::ZERO;
                for _ in 0..iters {
    ...
    group.bench_function(
        BenchmarkId::new("LRU - Scan Pollution", cache_capacity),
        |b| {
            b.iter_custom(|iters| {
                let mut elapsed = std::time::Duration::ZERO;
                for _ in 0..iters {
    ...
    group.bench_function(
        BenchmarkId::new("Moka W-TinyLFU - Scan Pollution", cache_capacity),
        |b| {
            b.iter_custom(|iters| {
                let mut elapsed = std::time::Duration::ZERO;
                for _ in 0..iters {
